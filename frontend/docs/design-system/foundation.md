...
顧客Previewは既存の`PageHeader`、`PageToolbar`、`DataTable`、`Tabs`、`Card`、`Badge`、Form、状態金型だけを組み合わせます。ページは`CustomerRepository`だけを参照し、Preview adapter、GAS client、storage、物理データ定義へ直接依存しません。顧客用のCopy、列・タブ・表示変換、Previewデータ、サービス契約はそれぞれCopy SSOT、page config、adapter、contractsに分離します。
