...
import { Button, Card, DataTable, EmptyState, PageHeader, Select, Skeleton, StatusMessage, Tabs, TextField, Textarea, type DataTableColumn, type SelectOption } from '../../components/ui';
import type { CustomerRepository, DiscordCustomerInviteResult, DiscordCustomerScaleOption, DiscordTicketResult, PaymentProfileDto, ShippingAddressDto } from '../../features/customers/contracts';
...
type ScaleSaveState = 'idle' | 'saving' | 'error';
...
export function CustomerDetailPage({ repository, canIssueDiscordTicket }: { repository: CustomerRepository; canIssueDiscordTicket: boolean }) {
  ...
  const [scaleOptions, setScaleOptions] = useState<readonly DiscordCustomerScaleOption[]>([]);
  const [scaleSaveState, setScaleSaveState] = useState<ScaleSaveState>('idle');
  ...
  useEffect(() => {
    if (!canIssueDiscordTicket) return;
    void repository.getDiscordCustomerScaleOptions().then(setScaleOptions).catch(() => {});
  }, [canIssueDiscordTicket, repository]);

  const handleScaleChange = useCallback(async (scaleKey: string) => {
    if (!customer) return;
    setScaleSaveState('saving');
    setScaleSaveError('');
    try {
      const result = await repository.updateDiscordCustomerScale(customer.profile.customerId, scaleKey);
      if (result.success) {
        setScaleSaveState('idle');
      } else {
        setScaleSaveState('error');
        setScaleSaveError(result.error ?? '');
      }
    } catch (cause) {
      setScaleSaveState('error');
      setScaleSaveError(cause instanceof Error ? cause.message : '');
    }
  }, [customer, repository]);
  ...
  const scaleSelectOptions: SelectOption[] = useMemo(() => scaleOptions.map((o) => ({ value: o.key, label: o.label })), [scaleOptions]);

  const currentScaleKey = useMemo(() => {
    if (!customer || scaleOptions.length === 0) return '';
    const labelValue = customer.profile.customerScale;
    const matched = scaleOptions.find((o) => o.label === labelValue);
    return matched ? matched.key : '';
  }, [customer, scaleOptions]);
