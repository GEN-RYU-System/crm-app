...
import { Badge, Button, Card, DataTable, EmptyState, PageHeader, Skeleton, StatusMessage, Tabs, TextField, Textarea, type DataTableColumn } from '../../components/ui';
import type { CustomerAggregateDto, CustomerRepository, PaymentProfileDto, ShippingAddressDto } from '../../features/customers/contracts';
...
export function CustomerDetailPage({ repository }: { repository: CustomerRepository }) {
  ...
  if (state === 'loading') return <>{header}<Badge variant="info">{customersCopy.previewBadge}</Badge><Card><Skeleton variant="list" rows={6} label={customersCopy.detailLoading} /></Card></>;
  if (state === 'missing') return <>{header}<Badge variant="info">{customersCopy.previewBadge}</Badge><EmptyState title={customersCopy.detailNotFoundTitle} description={customersCopy.detailNotFoundDescription} action={<Button onClick={() => navigate(customerListPath())}>{customersCopy.backToList}</Button>} /></>;
  if (state === 'error') return <>{header}<Badge variant="info">{customersCopy.previewBadge}</Badge><StatusMessage variant="error">{customersCopy.detailLoadErrorPrefix} {error}<Button variant="outline" onClick={() => void load()}>{customersCopy.retry}</Button></StatusMessage></>;
  const profile = customer!.profile;
  return <>{header}<Badge variant="info">{customersCopy.previewBadge}</Badge><Tabs aria-label={customersCopy.tabsLabel} items={CUSTOMER_DETAIL_TABS} activeKey={tab} onChange={setTab} variant="underline" size="md" />{tab === 'basic' && <Card>{CUSTOMER_PROFILE_FIELDS.map((field) => field.key === 'note' ? <Textarea key={field.key} label={field.label} value={displayCustomerProfileValue(profile, field.key)} readOnly /> : <TextField key={field.key} label={field.label} value={displayCustomerProfileValue(profile, field.key)} readOnly />)}</Card>}{tab === 'shipping' && <Card>{customer!.shippingAddresses.length === 0 ? <EmptyState title={customersCopy.shippingEmptyTitle} description={customersCopy.shippingEmptyDescription} /> : <DataTable ariaLabel={customersCopy.shippingTableLabel} columns={shippingColumns} rows={customer!.shippingAddresses} rowKey={(row) => row.addressId} />}</Card>}{tab === 'payment' && <Card>{customer!.paymentProfiles.length === 0 ? <EmptyState title={customersCopy.paymentEmptyTitle} description={customersCopy.paymentEmptyDescription} /> : <DataTable ariaLabel={customersCopy.paymentTableLabel} columns={paymentColumns} rows={customer!.paymentProfiles} rowKey={(row) => row.paymentProfileId} />}</Card>}</>;
}
