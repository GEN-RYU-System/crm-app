...
import { Badge, Button, Card, DataTable, EmptyState, PageHeader, PageToolbar, StatusMessage, TextField, type DataTableColumn } from '../../components/ui';
...
export function CustomerListPage({ repository }: { repository: CustomerRepository }) {
  ...
  const columns: readonly DataTableColumn<CustomerListRow>[] = CUSTOMER_LIST_COLUMNS.map((column) => {
    ...
    return { key: column.key, header: column.label, renderCell: (row) => row[column.key], ariaSort, onSort: () => changeSort(column.key), sortAriaLabel: customersCopy.sortLabel(column.label, direction), sortIcon: <SortIcon aria-hidden="true" />, cellAlignment: column.cellAlignment };
  });
  const isEmpty = state === 'ready' && filteredRows.length === 0;
  return <><PageHeader eyebrow={customersCopy.eyebrow} title={customersCopy.title} subtitle={customersCopy.subtitle} action={<Badge variant="info">{customersCopy.previewBadge}</Badge>} /><PageToolbar start={<TextField aria-label={customersCopy.searchLabel} placeholder={customersCopy.searchPlaceholder} value={query} onChange={(event) => setQuery(event.target.value)} fullWidth />} /><Card><Badge variant="info">{customersCopy.previewNote}</Badge>{state === 'loading' && <DataTable ariaLabel={customersCopy.tableLabel} columns={columns} rows={[]} rowKey={(row) => row.customerId} loading loadingLabel={customersCopy.loading} skeletonRows={4} />}{state === 'error' && <StatusMessage variant="error">{customersCopy.loadErrorPrefix} {error}<Button variant="outline" size="sm" onClick={() => void load()}>{customersCopy.retry}</Button></StatusMessage>}{isEmpty && <EmptyState title={query.trim() === '' ? customersCopy.emptyTitle : customersCopy.searchEmptyTitle} description={query.trim() === '' ? customersCopy.emptyDescription : customersCopy.searchEmptyDescription} />}{state === 'ready' && filteredRows.length > 0 && <DataTable ariaLabel={customersCopy.tableLabel} columns={columns} rows={filteredRows} rowKey={(row) => row.customerId} onRowClick={(row) => navigate(customerDetailPath(row.customerId))} />}</Card></>;
}
