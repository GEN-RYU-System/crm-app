...
export function LeadListPage({ canAdd }: { canAdd: boolean }) {
  ...
  return <><PageHeader eyebrow={leadsCopy.eyebrow} title={leadsCopy.title} subtitle={leadsCopy.subtitle} action={<div className="lead-list-page__actions"><Button onClick={() => void refreshAll()} loading={refreshing} loadingText={leadsCopy.refreshing}>{leadsCopy.refresh}</Button>{canAdd && <Button variant="secondary" onClick={() => navigate(LEAD_EDITOR_PATHS.create, { state: { leadType: activeType } })}>{leadsCopy.create}</Button>}</div>} /><div className="lead-list-page__tabs" role="tablist" aria-label={leadsCopy.title}>{LEAD_LIST_TABS.map(({ type, label }) => <Button key={type} variant="tab" active={activeType === type} role="tab" aria-selected={activeType === type} onClick={() => setActiveType(type)}>{label}</Button>)}</div>{refreshing && hasRecords && <StatusMessage variant="loading">{leadsCopy.refreshing}</StatusMessage>}{error && <StatusMessage variant="error">{leadsCopy.loadErrorPrefix} {error}<Button variant="outline" size="sm" onClick={() => void retryType(activeType)} loading={loading} loadingText={leadsCopy.retry}>{leadsCopy.retry}</Button></StatusMessage>}{!hasRecords && loading && <Card><Skeleton variant="table" rows={4} label={leadsCopy.loading} /></Card>}{hasRecords && rows.length === 0 && <EmptyState title={leadsCopy.emptyTitle} description={leadsCopy.emptyDescription} />}{hasRecords && rows.length > 0 && <Card><div className="lead-list-page__table-wrap"><table className="lead-list-page__table" aria-label={leadsCopy.tableLabel}><thead><tr>{LEAD_LIST_COLUMNS.map((column) => { const ariaSort = sort.key === column.key ? sort.direction : 'none'; const direction = ariaSort === 'none' ? leadsCopy.sortNone : ariaSort === 'ascending' ? leadsCopy.sortAscending : leadsCopy.sortDescending; return <th key={column.key} scope="col" aria-sort={ariaSort}><button type="button" className="lead-list-page__sort-button" onClick={() => changeSort(column.key)} aria-label={leadsCopy.sortLabel(column.label, direction)}>{column.label}</button></th>; })}</tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="lead-list-page__row" role="button" tabIndex={0} onClick={() => navigate(LEAD_EDITOR_PATHS.detailFor(row.id))} onKeyDown={(event) => onRowKeyDown(event, row.id)}>{LEAD_LIST_COLUMNS.map((column) => <td key={column.key}>{row[column.key]}</td>)}</tr>)}</tbody></table></div></Card>}</>;
}
