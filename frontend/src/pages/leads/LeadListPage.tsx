...
import { Button, Card, DataTable, DataTableToolbar, EmptyState, PageHeader, StatusMessage, TabBar, type DataTableColumn } from '../../components/ui';
...
export function LeadListPage({ canAdd }: { canAdd: boolean }) {
  ...
  return <><PageHeader eyebrow={leadsCopy.eyebrow} title={leadsCopy.title} subtitle={leadsCopy.subtitle} /><DataTableToolbar start={<TabBar aria-label={leadsCopy.title} items={LEAD_LIST_TABS.map(({ type, label }) => ({ key: type, label }))} activeKey={activeType} onChange={(type) => setActiveType(type)} />} end={<div className="lead-list-page__actions"><Button variant="secondary" onClick={() => void refreshAll()} loading={refreshing} loadingText={leadsCopy.refreshing}>{leadsCopy.refresh}</Button>{canAdd && <Button variant="primary" onClick={() => navigate(LEAD_EDITOR_PATHS.create, activeType === 'all' ? undefined : { state: { leadType: activeType } })}>{leadsCopy.create}</Button>}</div>} /><Card className="lead-list-page__data-card">{refreshing && hasRecords && <div className="lead-list-page__data-state"><StatusMessage variant="loading">{leadsCopy.refreshing}</StatusMessage></div>}{error && <div className="lead-list-page__data-state"><StatusMessage variant="error">{leadsCopy.loadErrorPrefix} {error}<Button variant="outline" size="sm" onClick={() => void retryType(activeType)} loading={loading} loadingText={leadsCopy.retry}>{leadsCopy.retry}</Button></StatusMessage></div>}{initialLoading && <DataTable ariaLabel={leadsCopy.tableLabel} columns={columns} rows={[]} rowKey={(row) => row.id} loading loadingLabel={leadsCopy.loading} skeletonRows={4} surface="embedded" />}{hasRecords && rows.length === 0 && <div className="lead-list-page__data-state"><EmptyState title={leadsCopy.emptyTitle} description={leadsCopy.emptyDescription} /></div>}{hasRecords && rows.length > 0 && <DataTable ariaLabel={leadsCopy.tableLabel} columns={columns} rows={rows} rowKey={(row) => row.id} onRowClick={(row) => navigate(LEAD_EDITOR_PATHS.detailFor(row.id))} surface="embedded" />}</Card></>;
}
