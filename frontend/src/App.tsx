...
import { ORDER_EDITOR_SEGMENTS } from './pages/orders/orderEditorConfig';
...
function AppRouter() {
  ...
  const ordersRoute = permissionState.status === 'checking' ? <StatusMessage variant="loading"><Spinner size="sm" aria-label={ordersCopy.permissionsChecking} />{ordersCopy.permissionsChecking}</StatusMessage> : canAccessOrders ? <OrderListPage canAdd={canAddOrders} /> : <Navigate to={NAVIGATION_BY_ID.dashboard.hash} replace />;
  const orderCreateRoute = canAccessOrders && canAddOrders ? <OrderEditorPage mode="create" canEdit={true} repository={orderGasRepository} customerRepository={customerGasRepository} /> : <Navigate to={canAccessOrders ? NAVIGATION_BY_ID.orders.hash : NAVIGATION_BY_ID.dashboard.hash} replace />;
  ...
  </Routes></AppShell></QuoteListCacheProvider></StaffListCacheProvider></OrderListCacheProvider></InventoryListCacheProvider></CustomerListCacheProvider></LeadListCacheProvider></HashRouter>;
}
