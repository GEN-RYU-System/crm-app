...
export function usePrefetch(permissions: NavigationPermissions | null): void {
  ...
  const { ensureLoaded: ensureInbox } = useInboxListCache();
  ...
  useEffect(() => {
    ...
    const steps: Array<{ canAccess: boolean; load: () => Promise<void> }> = [
      ...
      { canAccess: canAccessNavigationItem(NAVIGATION_BY_ID.inbox,       permissions), load: () => ensureInbox() },
    ];
  ...
  }, [permissions, ensureLeads, ensureLeadFormOptions, ensureCustomers, ensureAggregates, ensureInventory, ensureInventoryProductOptions, ensureOrders, ensureCurrencies, ensureSalesOrders, ensureStaff, ensureQuotes, ensureInbox]);
