...
const japanese = /[\u3040-\u30ff\u3400-\u9fff]/u; const copyDir = resolve(srcDir, 'content/ja'); const previewDir = resolve(srcDir, 'preview'); const rootErrorFile = resolve(srcDir, 'main.tsx'); for (const file of sourceFiles) { if (!['.ts', '.tsx'].includes(extname(file)) || file.startsWith(copyDir) || file.startsWith(previewDir)) continue; const source = await readFile(file, 'utf8'); if (file === rootErrorFile) { const withoutRootError = source.replace(/throw new Error\('[^']*'\);/, ''); if (japanese.test(withoutRootError)) violations.push(`Japanese copy outside content/ja: ${relative(frontendDir, file)}`); continue; } if (japanese.test(source)) violations.push(`Japanese copy outside content/ja: ${relative(frontendDir, file)}`); }
...
const SYNC_POLLER_EXEMPT_PROVIDERS = new Set([
  ...
  'InboxConversationListCacheProvider',
  // OrderListCacheProvider: SyncPoller の refreshers に refreshOrders として実登録済み。
  // ただし SalesOrderListCacheWithOrderSeed も useOrderListCache を呼ぶため、
...
const GAS_CLIENT_IN_PAGES_ALLOWLIST = new Set([
  ...
  'src/pages/data-management/OwnMasterPage.tsx',     // getCoreOwnCategories / upsertCoreOwnCategory etc. (master CRUD)
  'src/pages/data-management/ProductMasterPage.tsx', // getCoreSharedProducts / upsertCoreProductPackage etc. (master CRUD)
  'src/pages/data-management/ExportMasterPage.tsx',  // getCoreItems / upsertCoreItem etc. (master CRUD)
