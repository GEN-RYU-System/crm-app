...

{
  ...
  assert.equal(result.leadNonEmptyRecordCount, 5);
  assert.equal(result.emptyAssigneeIdCount, 2);
  ...
  assert.equal(result.emp00001CandidateCount, 2);
  assert.equal(result.emp00007CandidateCount, 3);
  ...
  ['阿部', '山田阿部', 'EMP-00001', 'EMP-00007', 'OTHER', 'unread'].forEach((value, index) => {
...

{
  const context = run({
    getEnvironment: () => 'development',
    getSpreadsheet: () => createSpreadsheet([['担当者ID', '担当者ID'], ['', '']])
  });
