...
| コミット | `981f1b1` |
| 理由 | PR #760 で `src/17_CountryMaster.js` を変更した際、同ファイル内の既存 JSDoc ブロック（`@returns` 例示 `'+81312345678'`、L342）とコードコメント（L375）が電話番号として誤検出された。 |
| 除外対象の例 | ` * @returns {string} '+81312345678'`, `// 例: '+81312345678'` |
...
| PR | #762（同上） |
| コミット | `981f1b1` |
| 理由 | PR #760 で `src/17_CountryMaster.js` を変更した際、`testNormalizePhone` 関数内のテストデータ配列（L447–457）に含まれる電話番号テストベクター（`['+81312345678', ...]`）が誤検出された。 |
| 除外対象の例 | `['+81312345678', 'JP', '0312345678'],` |
