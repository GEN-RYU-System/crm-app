...
| シート | 列名 | 型混在内容 | 状態 |
|-------|-----|---------|-----|
| 配送先マスタ | zip | string:2 / number:4（郵便番号の一部が数値型として格納） | ✅ **解消済み**（2026-09-02 全6行を文字列統一） |
| 支払先マスタ | zip | string:2 / number:4（同上） | ✅ **解消済み**（2026-09-02 全6行を文字列統一） |
| 商品マスタ同期 | Boxes per Case | string:19 / number:37（"-"文字列と数値が混在） | 未対応（別セッション管轄） |
| 商品マスタ同期 | Packs per Box | string:33 / number:62（"-"文字列と数値が混在） | 未対応（別セッション管轄） |
| 商品マスタ同期 | Case重量 | string:21 / number:71（"-"文字列と数値が混在） | 未対応（別セッション管轄） |
| システム設定 | 設定値 | number:2 / boolean:1（数値と真偽値が混在） | ✅ **解消済み**（2026-09-02 全3行を文字列統一: "30"/"true"/"2"） |

**解消済み列の変換詳細（2026-09-02 実施）:**
- zip: `setNumberFormat('@')` + `setValue(String(v))` で書式も文字列化。先頭0消失なし（10001/28001/75001/2000 は先頭0なし）
- setting_value: boolean→`"true"`（小文字）、number→`String(n)`。`getSettingValue` は `rawValue === 'true'` で判定済みのため互換性あり
- バックアップ: 配送先マスタ_backup_20260902_type / 支払先マスタ_backup_20260902_type / システム設定_backup_20260902_type

PostgreSQL移行時に列の型を固定する必要がある。`zip` は TEXT（解消済み）、`Boxes per Case` / `Packs per Box` / `Case重量` は NULL可の NUMERIC、`設定値` は TEXT（解消済み、アプリ側でパース）等の選択肢がある。
...
| E | システム設定をSQL移行するか | 3行。設定値列は **2026-09-02に文字列統一済み**（"30"/"true"/"2"）。更新日時はstring（ISO形式）。 | (a) SQLに移行してアプリ設定を統一 / (b) GASスプレッドシートに残す |
| F | zip列の型混在（配送先マスタ・支払先マスタ）をどう扱うか | ✅ **解消済み（2026-09-02）**: 選択肢(a)を実施。全6行を文字列に統一。PostgreSQL では TEXT 型で受け入れ可能。 | ~~(a) SQL移行前にスプレッドシートで文字列に統一~~ **完了** |
