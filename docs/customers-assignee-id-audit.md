...
1. 列名変更: 担当者ID → sales_assignee_id（PR-1）
2. Registry 追加: SALES_ASSIGNEE_ID（PR-2）
3. 値書き込み: 全6行に EMP-00001（谷澤 伸吾）を設定（PR-3）

### 実行PR

| PR | 内容 | mergedAt |
|----|------|---------|
| #850 | バックアップ関数追加 | 2026-09-01T08:09:00Z |
| #852 | バックアップ関数シート取得修正 | 2026-09-01T08:15:18Z |
| #853 | PR-1: 列名変更関数追加 | 2026-09-01T08:19:35Z |
| #854 | PR-2: Registry 追加 | 2026-09-01T08:23:33Z |
| #855 | PR-3: 値書き込み関数追加 | 2026-09-01T08:27:18Z |
| #857 | 現在値確認関数追加（診断用） | 2026-09-01T08:32:00Z |

### 検証結果

- バックアップ: 顧客マスタ_backup_20260901（7行 15列）
- dry-run: 6行・col11（sales_assignee_id）確認済み
- 書き込み結果: 全6行 EMP-00001 書き込み完了（`readCustomersAssigneeIdCurrent` で実測確認）
- Conformance Audit: 総不一致 0件（CUSTOMERS の差1列が解消、定義15/実15）

### 次フェーズの課題

営業担当者（sales_assignee_name）は ID 採用前の旧定義であり廃止予定。
