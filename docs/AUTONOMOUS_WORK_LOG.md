...
## 【受信箱公開 Phase 4-3】Config整合

- DEV実測のリード管理64列に合わせ、`HEADERS.LEADS` の13列目へ「流入元ID」、16列目へ「作品ID」を追加した。
- DEV実測の会話ログで「原文」と「翻訳文」の間にある「原文言語」を `HEADERS.CONVERSATION_LOG` の同位置へ追加した。
- 会話ログ末尾の「商談解析」は本指示で指定された追加対象ではないため、既存Configの対象範囲を変更しない。
- PR番号、squash merge SHA、revert SHA、デプロイ後検証はマージ後に追記する。
