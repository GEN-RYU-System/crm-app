...
## スプレッドシート列について（物理削除しない）

Discord連携廃止後も、以下の列はスプレッドシート上に残置する。
コードから参照されなくなるが、物理削除は破壊的かつ不可逆のため行わない。
...
| `Discord チャンネルID` | — |
| `Discord ユーザーID` | — |
| `Discrod 請求書 webhook` | **列名に誤字あり**（Discrod ← Discord の typo）|
| `Discrod 発送通知 webhook` | **列名に誤字あり**（Discrod ← Discord の typo）|

誤字（Discrod）はシートの実ヘッダー名のため、コード内も同表記で一致していた。
廃止後も列を残す方針のため、typo修正は行わない。
...
`顧客規模` 列はDiscord ロール付与のために使われていたが、列自体は顧客管理として有用なため残置。
ロール付与処理（PR #600 で削除済み）のみ廃止し、列は現行のまま。
