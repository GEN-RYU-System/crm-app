...
- リード管理のデータは CRM 本体に残す。顧客・担当・進捗・金額・日付などの分析に使うため、アーカイブブックへ複製しない。
- 会話ログだけを DEV 専用アーカイブブックへ移す。CRM 本体には、会話要約・最終会話日時・会話数などの分析用項目を残す。
- DEV アーカイブブックには、会話ログ用の5タブを作る。リード会話ログ_アーカイブ、商談会話ログ_成約、商談会話ログ_失注、商談会話ログ_追客、商談会話ログ_対象外。
- 各タブは、ログID、リードID、日時、送受信、発言者、原文、翻訳文、記録者ID、記録日時の9列を使う。
- PROD のデータをコピーまたは参照して DEV アーカイブを作らない。
