...
見積もりと請求書（オーダー）は入力内容が近いため、共通部品化を段階的に進める。

### 共通化のロードマップ

| 段階 | 対象 | 状態 |
|------|------|------|
| 段階1 | 明細入力（`LineItemEditor`） | 進行中 |
| 段階2 | 金額合計欄 | planned |
| 段階3 | 編集画面の骨組み | planned |
| 段階4 | 一覧画面の骨組み | planned |

★ 1段階ずつ、見積もりとオーダーの両方で実際に使ってから次の段階へ進む。

### 共通化しないもの

以下は画面ごとに個別実装を維持する。

- **入力項目の中身**: 見積もりはリード（`leadId`）、請求書は顧客（`customerId` + 配送先 + 支払先）
- **保存処理**: 呼び出す GAS 関数が異なる（`createCoreQuote` / `createCoreOrderForFrontend`）
...
## 15. 詳細/編集画面を作る時のチェックリスト

### GAS 側

1. **一覧キャッシュ（`createListCache`）があるか確認する**
   詳細画面で必要なデータが既存の一覧キャッシュに含まれているか確認する起点。

2. **詳細画面が使う項目を列挙する**
   画面に表示・編集する全フィールドをリストアップする。

3. **その項目が一覧データに含まれるか突き合わせる**
   - 含まれる → キャッシュから開ける（通信ゼロ）
   - 含まれない → 詳細取得 API が必要

4. **子データ（明細・配送先等）がある場合は別途取得する**
   一覧キャッシュには親レコードしか入っていないため、子データは別途 GAS 関数を用意する。

5. **書き込みがある場合は数式列を実測する（★人が確認）**
   `getFormulas()` で数式列がないか確認する。
   - 数式ゼロ → `setValues` 1 回にまとめる
   - 数式あり → `setValue` のセル単位書き込みに留める（`setValues` で数式が上書きされるため）

6. **書き込みは `withSheetWrite_` で包む**
   ロック（`LockService`）とキャッシュ削除を一括で行う（`src/00_SheetWrite.js`）。

### フロント側

7. **保存後は `refreshAll` を `await` しない**
   `refreshAll` は非同期で同期ポーリングに委ねる。`await` するとタイムアウトや UX ブロックが発生する。

```
★ 手順 5（setValues・withSheetWrite_）と手順 7（refreshAll の await しない）は
  check-design-system.mjs が機械的に検査する。
★ 手順 3 の突き合わせと、手順 5 の数式確認は人が行う（自動検査不可）。

★ 2 つ目の詳細画面を作る時、
  リードの実装と突き合わせて共通フックを抽出すること。
```
