...
選択肢マスタは英語スネークケース化の対象から外す。
36列中33列が画面に届いていないため、整形の実益がない。

### 到達しない理由の内訳（区分C 16列）

| 理由 | 列数 |
|---|---|
| DROPDOWN_OPTIONS 未定義バグ（27_WebApp.js:251） | 9列 |
| getDealReportDropdownOptions が dispatch 未登録（dead service） | 4列 |
| シート初期化用 setup 関数のみ（画面に出ない） | 3列 |

区分Cは「未実装」ではなく「壊れている」または「dead code」である。

### SSOT違反9件の扱い

すべてコード側のキー名誤り、またはシートに存在しない列への参照。
ただし参照元がすべて dead code または未定義バグで動作していないため、
**個別の修正は行わない**。再設計フェーズでまとめて解消する。

### 再設計フェーズで判断が必要な列

| 列 | 論点 |
|---|---|
| 役割 / ステータス（col 18,19） | setDataValidations() が担当者マスタの入力規則に使用。削除するとスプレッドシート直接編集時のプルダウンが消える |
| 仕入元（col 31、44値） | 44件の実データが蓄積されている。削除すると失われる |
| eLogiCSV格納フォルダ / ラベルPDF格納フォルダ（col 32,33） | 選択肢ではなく設定値（Drive フォルダURL）。削除ではなくシステム設定への移動が適切 |
| リード種別 / 返信速度（区分A） | 唯一 React 新pathで実動している2列。再設計後も必要 |
| アーカイブ理由（区分B） | 旧SPA で実動。新方式での要否は未確認 |

### 未解決のバグ（再設計フェーズで対応）

27_WebApp.js:251 の DROPDOWN_OPTIONS は未定義変数。
旧SPA の getDropdownOptions dispatch は常に undefined を返す。
レガシーページの機能であり新方式には影響しないため、
現時点では修正しない。
