...
| **選択肢マスタ（全36列）** | 36列中33列が画面に届いていない。列名整形ではなく再設計フェーズで扱う（PO決定 2026-08-31）。詳細は `docs/option-master-audit.md` 参照 |
...
| Buddy専用（追加確認済み: Section 14） | 2列 | Buddyフィードバック / Buddyメンテナンスメニュー表示 |
| 他機能も使用 | 2列 | 1回の発注金額 / 商談の手応え |
| 未参照 | 0列 | — |
...
| 7 | 他機能も使用の2列（1回の発注金額・商談の手応え）の扱い | Buddy廃止後も他機能（チャットUI・顧客一覧・営業フォーム）から参照されているため、除外前に当該機能の対応が必要。Buddyフィードバック・Buddyメンテナンスメニュー表示は追加調査でBuddy専用と確認（Section 14参照） |

---

## 14. 追加調査結果（Buddyフィードバック / Buddyメンテナンスメニュー表示）

**調査日**: 2026-08-31  
**背景**: Section 11 で「他機能も使用」と記録した 2 列について、参照元コードを精読し判定を確定した。

---

### Buddyフィードバック（リード管理）

**参照元**: `src/20_ReportService.js`

| 行番号 | コード断片 | 説明 |
|--------|-----------|------|
| L146 | `data[existingRow - 1][headers.indexOf('Buddyフィードバック')] \|\| ''` | 週次レポート更新時に既存の Buddy フィードバック値を保持（上書き防止） |
| L164 | `'', // Buddyフィードバック（後で更新）` | 新規レポート行作成時は空で初期化 |
| L317 | `const feedbackIdx = headers.indexOf('Buddyフィードバック');` | `generateBuddyFeedbackForReport()` 内で AI 生成フィードバックを書き込む |

**ファイルヘッダー（L1-15）**: "ReportService.gs / 週次・月次レポート機能を提供するサービス / Phase 6: レポート機能"

**判定: Buddy 専用**  
`20_ReportService.js` は Buddy ファイルリスト（Section 11 定義）には含まれないが、ファイル全体が Buddy 週次・月次レポート機能（Phase 6）の実装であり、`Buddyフィードバック` 列への全ての参照（L146 の保持・L164 の初期化・L317 の書き込み）は Buddy レポート処理に属する。他機能（非 Buddy 系）からこの列を参照するコードは確認されなかった。

→ **Section 11 の「他機能も使用」判定を「Buddy 専用」に修正**（集計反映済み）

---

### Buddyメンテナンスメニュー表示（担当者マスタ）

**参照元**: `src/32_StaffService.js`

| 行番号 | コード断片 | 説明 |
|--------|-----------|------|
| L482 | `const buddyMenuIdx = headers.indexOf('Buddyメンテナンスメニュー表示');` | メニュー設定読み取り関数内で 5 つのメニューフラグのうちの 1 つとして読み込み（チャット / 営業 / 設定 / Buddyメンテナンス / 管理者） |
| L570 | `let buddyMenuIdx = headers.indexOf('Buddyメンテナンスメニュー表示');` | メニュー設定書き込み関数内で同じく 5 フラグのうちの 1 つとして読み込み |
| L585 | `if (buddyMenuIdx === -1) columnsToAdd.push('Buddyメンテナンスメニュー表示');` | 列が存在しない場合に自動追加する処理 |

**frontend/src/ 参照**: 0 件（GAS 側が返す boolean `menuPreferences.buddyMaintenance` を使用）

**src/index.html 参照**: L14698, L16313, L16339（スタッフ設定 UI での Buddy メンテナンスメニュー表示制御）

**判定: Buddy 専用**  
管理するサービス（`32_StaffService.js`）は Buddy ファイルリスト外の汎用担当者サービスだが、この列の目的は「Buddy メンテナンスメニューを担当者ごとに表示/非表示にする」アクセス制御に特化している。`src/index.html` の参照箇所も Buddy メンテナンスメニューの UI 制御のみ。列自体の業務的用途は Buddy 機能に限定されており、Buddy 廃止後は不要になる列と判定できる。

→ **Section 11 の「他機能も使用」判定を「Buddy 専用」に修正**（集計反映済み）

---

### 判定一覧（Section 11 集計の根拠）
