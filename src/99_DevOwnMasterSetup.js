/**
 * 99_DevOwnMasterSetup.js
 *
 * 目的: 自社マスタ（自社大分類・自社作品・自社メーカー・自社商品）を
 *       DEVスプレッドシートに新設する。
 ...
 *
 * 使い方:
 *   clasp run setupOwnMasterSheets --params '["DRY_RUN"]'
 *   clasp run setupOwnMasterSheets --params '["APPLY"]'
 */

var OWN_MASTER_TABLE_KEYS = ['OWN_CATEGORIES', 'OWN_WORKS', 'OWN_MANUFACTURERS', 'OWN_PRODUCTS'];

/**
 * 自社マスタ4シートを DRY_RUN または APPLY する。
 *
 * @param {string} mode - 'DRY_RUN' または 'APPLY'
 * @returns {Object} 実行結果
 */
function setupOwnMasterSheets(mode) {
  ...
  if (getEnvironment() !== 'development') {
    throw new Error('setupOwnMasterSheets は development 環境でのみ実行できます。');
  }

  var ss = getSpreadsheet();
  var existingSheetNames = ss.getSheets().map(function(s) { return s.getName(); });

  var plan = OWN_MASTER_TABLE_KEYS.map(function(tableKey) {
  ...
  Logger.log('=== setupOwnMasterSheets (' + mode + ') ===');
  Logger.log('');
