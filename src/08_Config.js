...
const DROPDOWN_COLUMNS = [
  ...
  'リードステータス', '次回アクション日', '取り扱いタイトル', '販売形態', '競合比較中',
  '商談結果', 'アーカイブ理由', '対象外理由', '失注理由', '役割', 'ステータス', '期間タイプ'
];
...
const HEADERS = {
  ...
  // Phase 2 rename: 日本語列名 → 英語スネークケース（PR-1 両対応 / PR-2 シート変更 / PR-3 旧名削除）
  CONVERSATION_LOG: [
    'log_id',           // 1: 自動採番（LOG-00001）（旧: ログID）
    'lead_id',          // 2: LDI-00001 / LDO-00001（旧: リードID）
    'occurred_at',      // 3: メッセージの日時（旧: 日時）
    'direction',        // 4: 送信/受信（旧: 送受信）
    'speaker',          // 5: 担当者名 or 顧客名（旧: 発言者）
    'original_text',    // 6: オリジナルメッセージ（旧: 原文）
    'original_language',// 7: 原文の言語（旧: 原文言語）
    'translated_text',  // 8: 日本語訳（旧: 翻訳文）
    'recorded_by',      // 9: 記録した担当者ID（旧: 記録者ID）
    'recorded_at',      // 10: 記録した日時（旧: 記録日時）
    'deal_analysis'     // 11: 商談内容の解析結果（旧: 商談解析）
  ],
  // 専門用語辞書シート（5列）
  TERM_DICTIONARY: [
    '英語',           // 1: Box
  ...
  // オーダー管理シート（33列: 1オーダー=1行）
  // ★ Phase4で26→39、是正で進捗6列削除（発送タブのみで管理）→33列
  ORDER_MASTER: [
    'オーダーID',       // 1: OD-00001
    '請求書番号',       // 2: #0891
    '顧客ID',           // 3: CT-00001
    ...
    '受注担当ID',       // 27: EMP-ID参照
    '営業担当ID',       // 28: EMP-ID参照
    '発送担当ID',       // 29: EMP-ID参照（オーダー単位の主担当）
    '取引備考欄',       // 30 ← 旧col36（進捗6列削除後にシフト）
    '予約請求書番号',   // 31 ← 旧col37: #DEP 0002形式
    '発売予定日',       // 32 ← 旧col38
    'デポジット率'      // 33 ← 旧col39
  ],
  ...
  PURCHASE: [
    'purchase_id',           // 1: PUR-00001
    'order_id',              // 2: OD-* 空許容（紐づかない仕入れも可）
    'purchase_assignee_id',  // 3: EMP-* 担当者マスタ参照
    'ordered_at',            // 4
    'transaction_number',    // 5
    'supplier',              // 6: 文字列（将来の仕入先マスタ化を見据え）
    'supplier_url',          // 7
    'quantity',              // 8
    'unit_price',            // 9
    'amount',                // 10: 数量×単価 or 総額（元データ対応）
    'shipping_or_agency_fee', // 11
    'carrier',               // 12
    'tracking_number',       // 13: '@'テキスト書式
    'status',                // 14
    'note',                  // 15
    'registered_at',         // 16
    'updated_at'             // 17
