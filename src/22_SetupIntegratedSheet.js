...
/**
 * リード管理シートの49列ヘッダー定義
 * CoreSchemaRegistry の LEADS（51列）から新規シート作成時に使用するサブセット。
 * CLAUDE.md Section 2.2 準拠
 *
 * 変更履歴:
 *   2026-09-01: Buddy廃止に伴い定義外11列を除去（release/leads-delete-cols）
 *     削除列: 1回の発注金額・購入頻度・商談の手応え・Good Point・More Point・
 *             反省と今後の抱負・レポート提出日・レポート確認者・レポート確認日・
 *             レポートコメント・Buddyフィードバック
 */
const LEAD_SHEET_HEADERS = [
  ...
  'アサイン日',         // 23: 自動
  '最終対応者ID',       // 24: 自動
  // カテゴリ4: 営業（商談中）（10列）— Buddy複合2列・商談の手応え を除去
  ...
  '月間見込み金額',     // 32: 数値
  '競合比較中',         // 33: プルダウン（設定シート参照）
  'アラート確認日',     // 34: 自動
  // カテゴリ5: 営業（レポート）（6列）— Buddy専用7列を除去
  '商談結果',           // 35: プルダウン（設定シート参照）
  '対象外理由',         // 36: プルダウン（設定シート参照）
  '失注理由',           // 37: プルダウン（設定シート参照）
  '初回取引日',         // 38: 日付
  '初回取引金額',       // 39: 数値
  '累計取引金額',       // 40: 自動計算
  // カテゴリ6: アーカイブ（2列）
  'アーカイブ日',       // 41: 自動
  'アーカイブ理由',     // 42: プルダウン（設定シート参照）
  // カテゴリ7: 会話ログ連携（3列）— Buddyフィードバック を除去
  '会話要約',           // 43: 自動
  '最終会話日時',       // 44: 自動
  '会話数',             // 45: 自動
  // カテゴリ8: 重複管理（4列）
