/**
 * 流入元マスタ 列名リネーム用 DEV ユーティリティ
 *
 * devBackupLeadSourceMasterSheet()
 *   - 流入元マスタ を 流入元マスタ_backup_20260902 にコピー
 *   - 行列数とヘッダーを検証する
 *
 * devRenameLeadSourceMasterColumns()
 *   - 流入元マスタ の1行目（ヘッダー）を新列名に変更する
 *   - データ行（2行目以降）は一切変更しない
...
function devBackupLeadSourceMasterSheet() {
  var ss = SpreadsheetApp.openById(getSpreadsheet().getId());
  var src = ss.getSheetByName('流入元マスタ');
  if (!src) { throw new Error('シートが見つかりません: 流入元マスタ'); }

  var backupName = '流入元マスタ_backup_20260902';
...
function devRenameLeadSourceMasterColumns() {
  var ss    = SpreadsheetApp.openById(getSpreadsheet().getId());
  var sheet = ss.getSheetByName('流入元マスタ');
  if (!sheet) { throw new Error('シートが見つかりません: 流入元マスタ'); }
  ...
  for (var i = 0; i < headerRow.length; i++) {
    if (RENAME_MAP[headerRow[i]]) {
      ...
      renamedCount++;
    } else if (headerRow[i] !== '' && headerRow[i] !== 'source_id') {
