...
/**
 * 【DRY RUN / 読み取り専用】リード管理 62列目（リードステータス）の実測値調査
 * - 値の分布（行数つき）
 * - 「新規」→「新規リード」の変換対象件数
...
function dryRunLeadStatusConversion() {
  ...
  sortedKeys.forEach(function(k) {
    ...
    if (!targetSet[converted]) {
      nonConforming.push({ raw: k, converted: converted, count: distribution[k] });
    }
  });
  ...
  if (nonConforming.length === 0) {
    ...
    out.push('  判定: 不適合あり ✗');
    nonConforming.forEach(function(item) {
      out.push('  不適合: "' + item.raw + '" → "' + item.converted + '" (' + item.count + '行)');
    });
  }
  out.push('');
