/**
 * 99_DevQuoteLineConditionReader.js
 *
 * 目的: 見積もり明細（QUOTE_LINES）の CONDITION 列に存在する値を
 *       重複なしで一覧表示する（DEV 専用 / 読み取り専用）
 *
 * 禁止事項:
 *   - シートへの書き込み（setValue / setValues / appendRow 等）
 *   - PROD 環境での実行
 *   - CONDITION 以外の列（商品名・金額・顧客情報等）の出力
 *
 * 使い方:
 *   clasp run readQuoteLineConditions
 */

/**
 * 見積もり明細シートの CONDITION 列に存在する値と件数を返す。
 ...
 * }}
 */
function readQuoteLineConditions() {
  if (getEnvironment() !== 'development') {
    throw new Error('readQuoteLineConditions は development 環境でのみ実行できます。');
  }

  var ss        = getSpreadsheet();
  var tableDef  = getCoreSchemaV1Table('QUOTE_LINES');
  var sheetName = tableDef.sheetName; // '見積もり明細'
  ...
  var condColName = getCoreSchemaV1HeaderName('QUOTE_LINES', 'CONDITION');
