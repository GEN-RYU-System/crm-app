...
指示書の各手順について、次のいずれかを必ず書くこと。

- 実施した場合: 実行したコマンドと出力を示す
- 実施していない場合: 【未実施】と明記し、理由を書く

推測で「問題ないはず」「同型だから動く」と判断した場合は
【未実施】とする。実測していないものを ✅ と書いてはならない。

作業ログに ✅ を書く前に、その手順を完了させること。
後から実施して辻褄を合わせることは、記録の信頼性を損なう。

2026-09-02、PR #927 で ?preview の動作確認を実施せずに
「Evaluator: Skip」として PR を通した。
根拠は「他マスタページと同型だから動くはず」という推測だった。
また getDeployedSha と監査を実施する前に作業ログに ✅ を記録した。
以後、実測と推測を明確に区別すること。
