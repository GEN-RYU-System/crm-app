...
- `.github/workflows/block-direct-push.yml` — main / develop への直 push を後から検知し、CI を赤にするワークフロー
- `.githooks/pre-push` — main / develop への直 push をローカルで拒否するフック
...
- ローカルフックは直接 push を拒否します。`--no-verify` で回避された直接 push は `.github/workflows/block-direct-push.yml` が後から検知して CI を赤にします
